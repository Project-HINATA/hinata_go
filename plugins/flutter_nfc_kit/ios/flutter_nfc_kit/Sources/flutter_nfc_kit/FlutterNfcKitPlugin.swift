...
public class FlutterNfcKitPlugin: NSObject, FlutterPlugin, NFCTagReaderSessionDelegate {
    ...
    public func tagReaderSession(_ session: NFCTagReaderSession, didDetect tags: [NFCTag]) {
        ...
        if tags.count > 1, preferFelicaWhenMixed {
            ...
            let felicaTag = tags.first { tag in
                if case .feliCa = tag {
                    return true
                }
                return false
            }
            if hasIso14443, let felicaTag {
                detectedTags = [felicaTag]
            }
        }

        if detectedTags.count > 1 {
        ...
        let firstTag = detectedTags.first!
        ...
        session.connect(to: firstTag) { (error: Error?) in
            ...
            if self.checkNDEF, let ndefTag {
                ndefTag.queryNDEFStatus() { (status: NFCNDEFStatus, capacity: Int, error: Error?) in
                    ...
                    // Ignore NDEF errors and return the connected tag metadata.
                    returnConnectedTag()
                }
            } else {
                returnConnectedTag()
