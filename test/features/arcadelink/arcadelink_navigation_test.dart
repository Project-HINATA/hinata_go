...
void main() {
  testWidgets(
    ...
    (tester) async {
      ...
      final invocation = container.read(arcadeLinkInvocationProvider);
      addTearDown(() {
        router.dispose();
        invocation.clear();
        container.dispose();
      ...
      await http.runWithClient(
        () async {
          ...
          for (var entry = 0; entry < 3; entry++) {
            if (entry == 1) router.go('/arcadelink/shop/machine');
            if (entry == 2) {
              invocation.handleURL('https://link.neri.moe/t/shop/machine');
            }
            await tester.pumpAndSettle();
            expect(find.byType(ArcadeLinkMachineLoginPage), findsOneWidget);
            ...
            expect(find.byType(ArcadeLinkMachineLoginPage), findsNothing);
