...
// No requests leave the process. state-checks asserts recovery and stale-response handling.
import SwiftUI
import Foundation
...
struct ClipPreviewApp: App {
  ...
  var body: some Scene {
    WindowGroup {
      Group {
        if checked { Text("State checks passed") } else { MachineLoginView().environmentObject(model) }
      }
      .task {
